...
@section('main-content')
    ...
    <div id="printableArea" class="panel-body">
                        ...
                        <ol>
                            @foreach($transaction->purchases as $purchase)
                                <li>
                                    ...
                                    {{ $purchase->product->name }} ({{ $purchase->quantity }} {{ $purchase->product->unit }})
                                </li>
                            @endforeach
                        </ol>
                    </td>
                    <td class="text-center">
                        {{ settings('currency_code') }} {{ $transaction->net_total }}
                    </td>
        ...
        <table class="table table-bordered visible-lg" style="width: 50%; font-weight: bold;" align="right">
            <tr style="background-color: #F8F9F9; border: 1px solid #ddd;">
                <td style="text-align: right;"><b>{{ trans('core.total_amount') }}:</b></td>
                <td>{{ settings('currency_code') }} {{ twoPlaceDecimal($transactions->sum('net_total')) }}</td>
            </tr>
            <tr style="background-color: #F8F9F9; border: 1px solid #ddd;">
                <td style="text-align: right;"><b>{{ trans('core.total_tax') }}:</b></td>
                <td>{{ settings('currency_code') }} {{ twoPlaceDecimal($transactions->sum('total_tax')) }}</td>
            </tr>
            <tr style="background-color: #F8F9F9; border: 1px solid #ddd;">
                <td style="text-align: right;"><b>{{ trans('core.total_quantity') }}:</b></td>
