<div id="page-header" class="{{ settings('theme') ?? 'bg-gradient-1' }}">
    ...
    <div id="header-nav-right">
        ...
        <a class="header-btn tooltip-button hidden-xs" data-placement="bottom" href="{{ route('sell.pos') }}" title="POS" target="_BLINK">
            <i class="glyph-icon fa fa-desktop"></i>
        </a>

        <!--Notification-->
        <?php
            $products = App\Models\Product::orderBy('name', 'asc')->select('name', 'quantity', 'alert_quantity', 'unit')->take(5)->get();
            $alert_products = [];
        ...
        <div class="dropdown hidden-xs" id="notifications-btn">
            <a data-toggle="dropdown"
                ...
                @if(count($alert_products) > 0)
                     style="background-color: #FFE9 41CC !important;"
                @endif>
                <i class="glyph-ic on icon-linecons-megaphone"
                    @if(count($alert_products) > 0)
        ...
        <a class="header-btn tooltip-button hidden-xs" data-placement="bottom" id="logout-btn" href="{{ route('lock') }}" title="Lock">
