              ...
              <tbody>
                @foreach($transaction->purchases as $purchase)
                  <tr>
                    ...
                    <td class="text-center">
                        {{ bangla_digit($purchase->quantity) }}
                        {{ $purchase->product->unit }}
                    </td>

                    <td class="text-center">
                    @if($purchase->quantity > 0)
                        {{ twoPlaceDecimal($purchase->sub_total / $purchase->quantity) }}
                    @else
                        0
                    @endif
                    </td>
                    ...
                    <td class="text-center">
                      {{ twoPlaceDecimal($purchase->sub_total) }}
                    </td>

                    @if(settings('product_tax'))
                    <td class="text-center">
                      {{ twoPlaceDecimal($purchase->product_tax) }}
                    </td>
                    @endif

                    <td class="text-center">
                        {{ twoPlaceDecimal($purchase->sub_total + $purchase->product_tax) }}
                    </td>
                  </tr>
                @endforeach
                ...
                <tr>
                    <td colspan="{{ sellDetailsColSpanNumber() + 1 }}" style="text-align: right">
                        <b>
                            ...
                            ({{ orderTax() }}
                            of {{ $transaction->total + $transaction->total_tax - $transaction->invoice_tax }})
                        </b>
                    </td>
                    <td>
